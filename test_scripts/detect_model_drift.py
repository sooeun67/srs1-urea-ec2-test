...
- python3 detect_model_drift.py --start-time '2025-10-15 00:00:00' --end-time '2025-10-15 12:00:00'
"""
...
try:
    ...
    print(f"[ERROR] MAE 계산 중 오류 발생: {e}")
    mae = None
...
try:
    if mae is None or len(aligned) == 0:
        ...
        grouped = df_eval.groupby("NOx_bin", observed=False).apply(
            lambda g: pd.Series({
                "count": len(g),
                "ME": (g["target"] - g["pred"]).mean(),
                "MAE": (g["target"] - g["pred"]).abs().mean(),
                "RMSE": np.sqrt(((g["target"] - g["pred"])**2).mean()),
        ...
        grouped = grouped.sort_values(by="NOx_bin", ignore_index=True)
        print("\n[BINNED] Performance by NOx(target) bins:")
        print(grouped.to_string(index=False))
except Exception as e:
    print(f"[ERROR] 구간별 통계 계산 중 오류 발생: {e}")
    ...
    if col_urea_flow not in df.columns:
        ...
        if s_flow.empty:
            ...
            if isinstance(baseline_urea_flow_rate, (int, float)) and baseline_urea_flow_rate > 0:
                ...
                print(f"[FLOW] 베이스라인 유량({baseline_urea_flow_rate}) 대비 절감률: {saving_rate:.2%}")
            else:
                print("[FLOW] baseline_urea_flow_rate가 0이거나 유효하지 않아 절감률 계산 불가")

except Exception as e:
    print(f"[ERROR] 절감률 계산 중 오류 발생: {e}")
    saving_rate = None


#####################################################
########## Summary DataFrame (None 안전처리) ##########
#####################################################
urea_saving = float(saving_rate) if isinstance(saving_rate, (int, float)) else None
mae_value = float(mae) if isinstance(mae, (int, float)) else None
...
summary_df = pd.DataFrame(
    [{
        col_datetime: end_time_str,
        col_urea_saving_rate: urea_saving,
        col_snr_nox_mae: mae_value,
    }],
