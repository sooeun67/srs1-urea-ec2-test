...
def plot_bhi_trend(df, output_file="bhi_trend_srdd.png"):
    ...
    ax.set_title(
        f"SRDD Bag Filter Health Index (BHI) Trend\n"
...
def export_bhi_data(
    ...
    """
    BFT-BHI 데이터를 InfluxDB에서 CSV로 내보내기 (SRDD 플랜트)
    ...
    print("🔍 BFT-BHI InfluxDB 데이터 내보내기 시작 (SRDD)")
    print("=" * 60)
    ...
    try:
        ...
        if output_file is None:
            ...
            output_file = f"bhi_data_srdd_{timestamp}_{hours}h.csv"
        ...
        plot_file = f"bhi_trend_srdd_{timestamp}_{hours}h.png"
        ...
        if file_size_mb > 25:
            ...
            print(
                f"   2. git commit -m 'Add SRDD BHI data and trend: {output_path.name}'"
            )
...
def main():
    parser = argparse.ArgumentParser(
        description="BFT-BHI InfluxDB 데이터를 CSV로 내보내기 (SRDD 플랜트)"
    )
    ...
    if result:
        print(f"\n🎉 SRDD BFT-BHI 데이터 내보내기 완료!")
    else:
        print(f"\n❌ SRDD BFT-BHI 데이터 내보내기 실패")
